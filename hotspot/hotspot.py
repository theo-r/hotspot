...
def main():
    user_name = st.sidebar.selectbox(
        label="Select a user", options=users + ["All"],
        key="user_name"
    )
    ...
    st.sidebar.button("Run", key="first_run")

    df: pd.DataFrame = load_data()

    if "first_run" in st.session_state and "user_name" in st.session_state and "start" in st.session_state and "end" in st.session_state:
       render_page(df=df, user_name=user_name, start=start, end=end, dates_index=dates_index) 
...
def get_top_album_images(df: pd.DataFrame, user_name: str, start: datetime, end: datetime):
    if user_name == "All":
        return (
...
def get_listens_per_day(df: pd.DataFrame, user_name: str, start: datetime, end: datetime, dates_index: pd.DataFrame):
    if user_name == "All":
        dfs = [
...
def get_listens_by_hour_of_day(df: pd.DataFrame, user_name: str, start: datetime, end: datetime, num_tracks: int):
    if user_name == "All":
        return (
...
def render_page(df: pd.DataFrame, user_name: str, start: datetime, end: datetime, dates_index: pd.DataFrame):
    ...
    top_album_images = get_top_album_images(df=df, user_name=user_name, start=start, end=end)
    ...
    listens_per_day = get_listens_per_day(df=df, user_name=user_name, start=start, end=end, dates_index=dates_index)
    listens_by_hour_of_day = get_listens_by_hour_of_day(df=df, user_name=user_name, start=start, end=end, num_tracks=num_tracks)
